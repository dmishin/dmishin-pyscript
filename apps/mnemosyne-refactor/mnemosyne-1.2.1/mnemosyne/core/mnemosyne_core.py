...
import shutil, datetime, bz2, copy, cStringIO, locale 
...
if sys.platform == 'darwin':
    ...
    try:
        ...
        pass
    basedir = os.path.join(home, ".mnemosyne")
...
# StartTime
#
#   Here we will store two different things:
#     - a timestamp for compatibility with older versions of Mnemosyne.
#     - a datetime.date object for proper time local zone handling in newer
#       versions.
#
##############################################################################
...
# Item
#
#   Note: self.unseen is needed on top of self.acq_reps, because the initial
#   grading of a manually added card is counted as the first repetition.
#   An imported card has no such initial grading, and therefore we do the
#   initial grading the first time we see it during the interactive learning
#   process. Because of this, determining if a card has been seen during the
#   interactive learning process can not be decided on the basis of
#   acq_reps, but still that information is needed when randomly selecting
...
class Item:
    ...
    def filtered_q(self):
        ...
        if "filter_q" in function_hooks:
            for f in function_hooks["filter_q"]:
                q = f(q, self)
    ...
    def filtered_a(self):
        ...
        if "filter_a" in function_hooks:
            for f in function_hooks["filter_a"]:
                a = f(a, self)
            ...
            for i in items:
                if type(i.id) != type("string") and \
                   type(i.id) != type(u"unicode"):
                    i.new_id()
            header_line = "Format Version 2 ---"

        # Add item.unseen if missing.

        if header_line.endswith("Format Version 2 ---"):
            for i in items:
                if i.acq_reps <= 1 and i.ret_reps == 0 and i.grade == 0:
                    i.unseen = True
                else:
                    i.unseen = False
...
def import_wcu(filename, default_cat, reset_learning_data=False):
    ...
    #def importwcu(file):
    #    wcuwalk(minidom.parse(file).documentElement,imported_items)
...
def rebuild_revision_queue(learn_ahead = False):
    ...
    if "after_scheduled_queue" in function_hooks:
        for f in function_hooks["after_scheduled_queue"]:
            revision_queue = f(revision_queue)
    ...
    # but which we have seen before. Also limit the queue size to 10 in
    # order not to have too large a spread.
    
    grade_0 = (i for i in items if i.is_due_for_acquisition_rep() \
                      and i.lapses == 0 and i.unseen == False and i.grade == 0)
    ...
    grade_1 = [i for i in items if i.is_due_for_acquisition_rep() \
                      and i.lapses == 0 and i.unseen == False and i.grade == 1]
